...
- **映射发生在生成时一次性完成**；生成后再改时间线（裁切/变速/分割），已生成
  ...
  **2026-09-26 用户：跟随「先不用」**，限制照旧（[生成的字幕长什么样](../architecture/subtitle-generation-style.md) 第八节）。
- **变速 clip**：源区间转写 → `timelineTime(atSource:)` 换算，天然正确；
  分段约束在时间线时间上执行（第 6 节）。
- **多个视频片段**：按素材分组成批转写；同一时间窗多个素材同时出声（主轨+音频轨
  重叠）时，各素材的 cue 都投影到时间线，**V1 全部保留**，渲染按第 9 节的
  排序合同处理（SRT 允许时间重叠）。
  **2026-09-26 起改为只留一条**（用户：「生成的字幕有时候会有重叠」）：谁识别得更清楚留谁、零碎杂音不要，
  外加面板上「只用选中的片段」 —— 见 [生成的字幕长什么样](../architecture/subtitle-generation-style.md) 第六、七节。
...
> **2026-09-26 起断句、去标点、一行多长、显示时间都按主流字幕规范重写**，现行规则以
> [生成的字幕长什么样](../architecture/subtitle-generation-style.md) 为准（本节保留当时的设计思路：先映射、后约束、
> 变速下同一素材分段不同、无解如实告警，这几条没变）。
...
> 2026-09-26 起**生成出来的字幕不再互相重叠**（几段素材同时有字只留一条）；这条渲染合同仍管用户手打、
> 导入的字幕叠在一起时怎么显示。
...
- **命名**：`<视频名>.<lang>.srt`（如 `movie.zh-Hans.srt`、`movie.en.srt`）；
  ...
  > **2026-09-24 更正**：导出面板改版后，`<视频名>` 就是面板上的标题，写进面板上的
  > 「导出至」（不再另选文件夹）；同名改成**先提示、再确认替换**，和视频同一条规则 ——
  > 追加 `-2/-3` 重导几次就堆出一串，还和视频配不上对。见
  > [导出设置](../architecture/export-settings.md) 第五节。
...
2. 分段默认阈值（行宽/时长/CPS/停顿）—— 可调实现已落
   `SubtitleSegmentationConfig`，默认值需产品拍板。**2026-09-26 已拍**：按主流规范（Netflix），
   见 [方案](2026-09-26-subtitle-generation-style.md)。
3. 生成后时间线再编辑 cue 不自动跟随、需手动重新生成 —— 产品体验是否接受
   待实际试用反馈。**2026-09-26 用户：跟随「先不用」。**
