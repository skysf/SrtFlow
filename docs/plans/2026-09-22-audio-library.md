...
`remoteKey` 非空时**插进第一层之前**：直接按 id 从 R2 重新拉，用户根本不会看到
"素材丢失"。这是 remote 素材相对本地素材的唯一优势，不用白不用。
...
| 层 | 做法 | 判断 |
| --- | --- | --- |
| 0 | `Audio/` 前缀**不走** `downloads.skylu.ai` 公开域名 | 地基，没这条下面全白搭 |
| 1 | `audio.skylu.ai` → Worker（R2 binding），每 IP 限速 | **唯一可靠的** —— 不依赖密钥保密 |
| 2 | manifest 不含真实 URL，`POST /grant {id}` 换 15 分钟签名 URL | 配合限速，批量成本陡增 |
| 3 | App 请求带 HMAC | **不做** —— 密钥在二进制里，`strings` 一扫就出来 |

目标是**批量拖不走**，接受**单首能被下走**。素材本身多是 CC0 / Pixabay License，
原件谁都能免费下 —— 真正值钱的是**选曲和 tags**，而那份 manifest 更好保护。

**Worker 最容易写错的一处**：Range 请求必须透传（`R2Bucket.get(key, {range})`），
`206` + `Content-Range` 要手拼。写错的话流播会退化成整段下载，而表面上看不出来。

Worker 免费额度每天 10 万请求，`AVPlayer` 流播一首会发好几个 Range —— 用户量上来要算。
