...
| 入口 | 手势 | 界面 |
| --- | --- | --- |
| 时间线 cue 块 | 单击选中 / **双击**改字 | 块上方的浮层（`SubtitleInlineEditor`） |
| 预览画面上的字幕 | 单击选中 / **双击**改字 | 字幕**下方**浮出输入框（放不下就翻到上方） |
| 右栏字幕表 | 常驻，行内编辑 | `VideoEditSubtitlePanel` + `VideoEditSubtitleCueRow` |

1. **写入只有一条路：`VideoEditProjectSubtitleLink` 的合同入口。** 三个入口都不许
   自己动 `state.subtitle` —— 一次提交 = 一步撤销，译文过期标记、置信度作废、
   两轨同步全靠合同（规则本体在 SrtFlowCore 的 `LinkedSubtitleEditing`，那边有单测）。
   新增一行是**合同 7**（`linkedInsertCue` → `LinkedSubtitleEditing.insertCue`）：
   只进原文轨，译文轨不跟着插空条（镜像是子集关系，空译文会在双语预览里多出一行）。
2. **选择只有一份**（`EditSelection`）：三个入口共用它，所以时间线上双击一条，
   右栏会滚过去并高亮，预览上出现的也是同一条的拖框。
3. **隐藏 = 不可编辑**，这条从时间线行扩展到全部入口：原文眼关着就不给原文输入框，
   译文眼关着就不给译文输入框，两只都关时字幕表整列置灰并说明原因（表头就放着
   那两只眼睛，让用户当场能开回来）。
4. **提交时机与仓库其余输入框一致：回车或失焦即提交**（`TimecodeField`、Inspector
   数值框同款），没有单独的「取消」—— 改错了按 ⌘Z。**一格 = 一步撤销**：原文和译文
   是两格，各自一步（一次 Done 里改了两格就是两步 —— 失焦提交与「整个会话一笔事务」
   不可兼得，这里选前者，它是不丢字的第一道防线）。
...
- [x] **右栏字幕表**：跟随播放高亮当前句；+ 在播放头新建一行并把光标放进去；
      一次编辑一步 ⌘Z（先退文字，再退新建）。
- [ ] **有译文的工程**：两条轨都在时，三个入口都要给出原文 + 译文两个输入框；
      关掉一只眼睛后对应的输入框消失/置灰，另一条仍可编辑。
