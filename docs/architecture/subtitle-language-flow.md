...
7. **可听快照是检测的唯一素材来源。** 音轨 metadata 查询和探针抽取都只能
   ...
   `lane.isHidden` 跳整轨、**按 V 藏起来的段**（`ClipVisibility.visible`）跳过、
   `isMuted || volume <= 0` 跳 clip、静帧图片段不算）。
   ...
   - **「读得出来」也不等于「有人说话」**（2026-09-26，[案例](../bugfixes/2026-09-26-auto-detect-probes-sound-effects.md)）：
     探针候选**长的先**（旁白往往长、音效短；一样长的保持快照原序），每段先拿一个已装语言
     （`SpeechTranscriptionService.screeningLocale`）短转写听一听，词数够门槛才用它检测，没有就换下一段，
     最多听 6 段；都没听出人声就退回第一段读得出来的、如实报检测不出来。听的时候转写栈出错直穿，
     不许当成素材读不了吞掉。反例：南极工程主轨第一段是 6 秒船撞冰音效，拿它检测每个候选都是 0 分。
   - **取消要在「吞掉素材错误」之前问。** 跳过逻辑天生会吃掉一串错误：用户按
     Stop 之后剩下的候选照样各抛一个 `ReadError`，全被吞掉就走到 `return nil`，
     调用方把 nil 翻成「素材都读不了」—— 任务终态从 `.cancelled` 变成
     `.failed`，用户明明是自己取消的（2026-08-09 第三轮复审）。三个点都查：
     **每轮开头**（已取消就一次抽取都不发起）、**吞错误之前**、
     **返回 nil 之前**。最后那一处把 nil 的含义收窄成「确实全都读不出音频」，
     调用方的 nil→失败文案转换就永远不会冤枉取消 —— 顺序错误被结构消掉，
     而不是靠调用方记得先查。两条取消通道都要问：任务自己的
     `token.isCancelled` 和结构化的 `Task.checkCancellation()`。
