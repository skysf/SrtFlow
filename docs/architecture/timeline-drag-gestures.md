...
- **缩放只有 `VideoEditProject.setPixelsPerSecond` 一个入口**，夹进
  ...
  捏合、工具栏按钮（⌘− / ⌘=，每下 ×1.5）、对数刻度的滑杆都经 `TimelineZoom.horizontal` 走到它，
  **同一拍把锚点拉回原位**：捏合 / Ctrl + 滚轮钉指针底下那一刻，工具栏三样钉播放头（不在视口里钉视口正中）。
  纵向缩放（⌥ 捏合、⌥ + Ctrl + 滚轮、⌘↓ ⌘↑）走 `TimelineZoom.vertical`，视频轨和音频轨统一成一个高度。
  约束全在 [捏合缩放](timeline-pinch-zoom.md)。
...
- **捏合缩放也不例外**（2026-09-26）：以前它按坐标 hitTest 现找滚动视图、自己推滚动，被当成「事件监视器里
  拿不到视图树」而豁免 —— 其实桥就挂在时间线里，拿得到这一份。它传错了坐标系（翻转的根视图），找的是
  上下镜像的那一处，锚点从来没生效（[案例](../bugfixes/2026-09-26-pinch-zoom-anchor-never-applied.md)）。
  现在指针换算走 `location(ofWindowPoint:in:)`、推滚动走 `keepAnchored`，豁免删了（第 9d 条）。
...
- **轨道头列和标尺钉住不动**，各自减/加**同一个**纵向滚动量（轨道头 `-offsetY`、
  ...
  **「盖在上面」的层级要在 `VStack` 那一层给**（`laneReorderOffset(…, pinnedOnTop: row.isRuler)`）：
  叠放次序只认最外面那一层 `.zIndex`，每一行都挂着的换位位移会把标尺自己写的层级盖成 0，
  滚下去之后块画到标尺上面（2026-09-26，[案例](../bugfixes/2026-09-26-tracks-cover-pinned-ruler.md)）。
  轨道头列的标尺那一行（总推子）同样钉住（`+offsetY` 抵掉整列的 `-offsetY`，垫同样高的不透明底）。
