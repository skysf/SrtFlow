...
音频不动的理由：把用户的 BGM 悄悄切成两半、还多出一个看不见的切口，比
「切口之后的音频要自己挪」更具破坏性；主流剪辑器的定格同样只推视频轨；
规则也和字幕/形状不动统一成一句话。代价写在明处：切口之后对好点的解说/
音效会比画面早一个定格时长，要不要顺推由用户自己决定。字幕不动另有一层
理由：那是链接进来的字幕文件，改它等于改那份 SRT。

> 历史：2026-08-23 之前主轨定格是「一次真正的插入编辑」—— 跨切口的音频
...
（`TimelineState.transitionOverlap`），定格把目标切短之后那个上限会跟着缩，
于是 `packMain()` 让后面的画面移动的距离**不等于**定格时长，而当年联动的
音频只会老老实实顺推一个定格时长 —— 实测（`checks/FreezeFrame` 钉住）：
A 在 10s 结束、与 B 有 1s 转场，在 A 的 8.5s 定格 → 画面位移 **2.325s**、
音频位移 **2.0s**，错位 **0.325s**。

§4 的改版把音频联动整个去掉之后，这条错位的前提不存在了 —— 定格和 ⌘B
分割一样不再承诺声画同步。

**2026-09-20 放开**（用户拍板，对齐剪映）：只禁**叠化区那一段**，不禁整个片段。
`participatesInMainTransition` 已从 `isFreezeEligible` 里摘掉，函数本身留着
（仍是个有意义的查询）。叠化区内禁定格是另一条独立规则，**必须留着** ——
那一帧画面是两段合成出来的，从单个源素材抽帧必然对不上。

放开之后的代价是可接受的：定格切短目标后转场会被重新限长，后面画面移动的距离
不等于定格时长。但**存的时长不丢** —— 容量只夹生效值
（`VideoEditTransitionHandles.effectiveTransitionDuration`），片段再变长转场就
自己回来了。
