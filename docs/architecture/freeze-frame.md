...
分割一样不再承诺声画同步。禁用**先保留**：定格切短目标后转场被重新限长，
后面画面移动的距离不等于定格时长，落点不好预期。要放开是另一个产品决定，
到时候把 `participatesInMainTransition` 从 `isFreezeEligible` 里摘掉即可
（叠化区内禁定格是另一条独立规则，必须留着，见 §5）。
